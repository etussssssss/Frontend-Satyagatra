...
class AdapterAdminButuhPersetujuanOnline(private var data: List<DataPendaftaran>) : RecyclerView.Adapter<AdapterAdminButuhPersetujuanOnline.ButuhPersetujuanOnline>() {

    inner class ButuhPersetujuanOnline(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ButuhPersetujuanOnline {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_admin_butuhpersetujuan_1, parent, false)

        return ButuhPersetujuanOnline(view)
    }


    @RequiresApi(Build.VERSION_CODES.O)
    override fun onBindViewHolder(holder: ButuhPersetujuanOnline, position: Int) {
        ...
        holder.buttonPilihKonselor.setOnClickListener {
            showPilihKonselorDialog(holder.itemView.context, datas)
        }
    ...
    private fun showPilihKonselorDialog(context: Context, dataUser: DataPendaftaran) {
...
//db.collection("chats").document(nomorDaftar).collection("messages")
//    .add(firstChat).addOnSuccessListener {
//
...
//            val intent = Intent(holder.itemView.context, PilihKonselor::class.java)
